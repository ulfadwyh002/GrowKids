...
# =====================================
# LOAD MODEL — dengan pengecekan path
# =====================================
...
if not os.path.exists(MODEL_PATH):
    ...
    st.info("Pastikan folder **model/** berisi **knn_stunting.pkl** di repo GitHub kamu.")
elif not os.path.exists(SCALER_PATH):
    st.error(f"❌ File scaler tidak ditemukan di: `{SCALER_PATH}`")
    st.info("Pastikan folder **model/** berisi **scaler.pkl** di repo GitHub kamu.")
else:
    try:
        ...
        st.error(f"❌ Gagal membaca file model: {e}")

# Hentikan app jika model belum siap
if not (model_loaded and scaler_loaded):
    st.warning("⚠️ Aplikasi tidak dapat berjalan karena model belum tersedia.")
    st.stop()
...
- Kunjungi Posyandu minimal 1x sebulan untuk pemantauan rutin
        """
    else:
        detail = """
...
- Minta rujukan ke **Puskesmas atau RSUD** terdekat untuk pemeriksaan lanjutan
        """
...
# =====================================
# SESSION STATE — riwayat prediksi
# =====================================
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("⚠️ Hasil prediksi **bukan** diagnosis medis. Selalu konsultasikan ke tenaga kesehatan profesional.")
...
if menu == "🔍 Prediksi":
    ...
    st.markdown("""
    Masukkan data anak di bawah ini. Hasil prediksi akan muncul setelah menekan tombol **Prediksi**.
    ...
    if predict_btn:
        ...
        # Z-Score otomatis dari WHO
        z_score = get_zscore(gender, age, height)

        # 8 fitur sesuai training model
        data = np.array([[
            gender_enc,
            age,
            birth_weight,
        ...
        Z-Score <strong>{z_score}</strong> menunjukkan tinggi badan anak {keterangan_z}
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("Apa itu Stunting?"):
            ...
            Anak dikategorikan stunting jika **Z-Score tinggi badan per usia (TB/U) < -2 SD** dari standar WHO.
        ...
        st.markdown('<p class="footer-note">⚠️ Prediksi ini bukan diagnosis medis. Selalu konsultasikan ke tenaga kesehatan profesional.</p>', unsafe_allow_html=True)
...
# =====================================
    ...
    **GrowKids** adalah aplikasi prediksi risiko stunting berbasis kecerdasan buatan (AI)
    ...
    > Recall 86% diprioritaskan karena dalam konteks skrining medis,
    > melewatkan kasus berisiko lebih berbahaya daripada false positive.
